...
- (void)loadAdWithType:(GappingADType)type
               withKey:(NSString *)key
              delegate:(id<GappingLoaderDelegate>)delegate;

//플로팅 광고를 노출 하고자 하는 경우 사용
//FloatingAd defult content margin (x: kFloatingAdMarginOffsetFromLeftSide, y: kFloatingAdMarginOffsetFromBottomSide)
//FloatingAd defult content ratio kFloatingAdContentDefaultRatio
- (void)loadFloatingAdWithAlign:(GappingFloatingADAlign)align
                        withKey:(NSString *)key
                       delegate:(id<GappingLoaderDelegate>)delegate;

//플로팅 광고를 노출 하고자 하는 경우 사용 (상세 노출 설정)
//FloatingAd content margin offset (x, y)
// x : horizontal margin from left-side ( kFloatingAdMarginOffsetMin <= x <= kFloatingAdMarginOffsetMax)
// y : vertical margin from bottom-side ( kFloatingAdMarginOffsetMin <= y <= kFloatingAdMarginOffsetMax)
// ratio : content ratio (kFloatingAdContentMinRatio <= r <= kFloatingAdContentMaxRatio)
- (void)loadFloatingAdWithAlign:(GappingFloatingADAlign)align
                        withKey:(NSString *)key
              withContendMargin:(CGPoint)offset
               withContentRatio:(CGFloat)ratio
                       delegate:(id<GappingLoaderDelegate>)delegate;


//VR 광고 시 광고 노출 영역을 설정하여 광고를 노출하는 경우 사용
//enableTimer : 가상광고를 반복적으로 노출 설정 값
- (void)loadVRAdWithAlign:(GappingVRADAlign)align
                  withKey:(NSString *)key
                 delegate:(id<GappingLoaderDelegate>)delegate
              enableTimer:(BOOL)enable;
...
- (void)loadBannerAdWithKey:(NSString *)key
                   delegate:(id<GappingLoaderDelegate>)delegate;

- (void)loadBannerAdWithAlign:(GappingBannerADAlign)align
                      withKey:(NSString *)key
                     delegate:(id<GappingLoaderDelegate>)delegate;

// 320*50 고정형 배너
- (void)loadFixedBannerAd:(NSString *)key
                 delegate:(id<GappingLoaderDelegate>)delegate;

// 320*100 고정형 배너
- (void)loadFixedLargeBannerAd:(NSString *)key
                      delegate:(id<GappingLoaderDelegate>)delegate;
